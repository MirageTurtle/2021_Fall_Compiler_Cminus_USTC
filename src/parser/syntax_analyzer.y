...
%token <node> DIGIT INTEGER
%token <node> FLOAT
%token <node> LETTER ID
%token <node> INT VOID
%token <node> LPAREN RPAREN LBRACKET RBRACKET LBRACE RBRACE COMMA SEMICOLON
%token <node> MINUS MUL DIV
%token <node> LT LE BT BE EQ NEQ
%token <node> ASSIGN
%token <node> IF ELSE RETURN WHILE
...
expression-stmt : expression SEMICOLON {$$ = node("expression-stmt", 2, $1, $2);}
        ...
        ;
selection-stmt : IF LPAREN expression RPAREN statement %prec LOWER_THAN_ELSE {$$ = node("selection-stmt", 5, $1, $2, $3, $4, $5);}
